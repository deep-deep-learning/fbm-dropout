...
class DenseNetFBM_with_branching(nn.Module):
    ...
    def __init__(self, hidden_sizes, n_agents, agent_len, max_iter, scale, grid_sizes, show=False, device=None, dtype=None):
        ...
        self.linear_2 = nn.Linear(hidden_sizes[0], hidden_sizes[1], device=device, dtype=dtype)
        self.linear_3 = nn.Linear(hidden_sizes[1], 10, device=device, dtype=dtype)
        self.dropout_1 = DropoutFBM_with_branching(0.9, n_agents[0], agent_len, max_iter, scale, grid_sizes[0], show=show, device=device, dtype=dtype)
        self.dropout_2 = DropoutFBM_with_branching(0.9, n_agents[1], agent_len, max_iter, scale, grid_sizes[1], show=show, device=device, dtype=dtype)
